...
class Logger():
    ...
    def update_stats(self, decision, clean_sequence, sequence_remains):
        ...
        self.stats['average_time_per_bag'] = hms_mean([i for i in self.stats['bag_times']])
        self.stats['t2t_mean'] = hms_mean([i  for i in self.stats['t2t_list']])
    ...
    def process_next_iter(self, decision, clean_sequence, sequence_remains):
            ...
            #newly added
            self.stats['transitions'].append(last_event_time)
            self.stats['total']['events'].append({'event': "t1" , 'time':last_event_time})
